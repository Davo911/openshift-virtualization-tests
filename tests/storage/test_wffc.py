...
def test_wffc_upload_dv_via_token(
    ...
    with upload_image_to_dv(
        ...
        dv.wait_for_dv_success()
        with create_vm_from_dv(dv=dv, vm_name=dv_name,os_flavor=OS_FLAVOR_FEDORA,  memory_guest=FEDORA_VM_MEMORY_SIZE,wait_for_cloud_init=True) as vm_dv:
            check_disk_count_in_vm(vm=vm_dv)
...
def test_wffc_import_http_dv(data_volume_multi_wffc_storage_scope_module):
    with create_vm_from_dv(
        dv=data_volume_multi_wffc_storage_scope_module, vm_name=data_volume_multi_wffc_storage_scope_module.name,os_flavor=OS_FLAVOR_FEDORA,  memory_guest=FEDORA_VM_MEMORY_SIZE,wait_for_cloud_init=True
    ) as vm_dv:
        check_disk_count_in_vm(vm=vm_dv)
...
def test_wffc_clone_dv(data_volume_multi_wffc_storage_scope_module):
    with create_dv(
        ...
        with create_vm_from_dv(dv=cdv, vm_name=cdv.name,os_flavor=OS_FLAVOR_FEDORA,  memory_guest=FEDORA_VM_MEMORY_SIZE,wait_for_cloud_init=True) as vm_dv:
            check_disk_count_in_vm(vm=vm_dv)
